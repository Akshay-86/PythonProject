...
def normalize(src: np.ndarray, dst: np.ndarray | None = None,dtype: int = -1,mask: np.ndarray | None = None)-> np.ndarray:
    ...
    '''
        0-1 normalization scales the pixel intensity values of an image to a floating-point range of 0.0,1.0, where the minimum value in the original image becomes 0.0 and the maximum value becomes 1.0. 
    ...
    disp = (outimg * 255).astype(np.uint8)
    return disp
